...
export default function AICommandCenter({
  ...
  const handleQuerySubmit = (e: React.FormEvent) => {
    ...
    if (trimmed.includes('销售') || trimmed.includes('销售额') || trimmed.includes('今日销售')) {
      ...
      setCommandLogs(prev => [
        ...
        { id: Date.now().toString() + '-resp', text: `⚠️ 校验到不满足系统支持的命令。支持快捷执行和模糊匹配销售、订单、库存、利润等指令。`, type: 'error' }
      ]);
    }
  };
  ...
  return (
    <div id="ai-cmd-overlay" className="fixed inset-0 z-50 flex justify-end bg-slate-950/40 backdrop-blur-xs">
      
      {/* Back click close area */}
      <div className="flex-1" onClick={onClose}></div>

      {/* Primary Drawer Panel */}
      <div 
        id="ai-cmd-drawer" 
        className="w-full max-w-md md:w-[480px] bg-slate-900 border-l border-slate-800 h-full flex flex-col shadow-2xl relative text-slate-100"
      >
        {/* Drawer Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div className="space-y-2 font-mono text-xs text-slate-300">
                ...
                <button 
                  onClick={() => {
                    ...
                    addLog('Sophia (CEO)', 'Manual Command Creation', `Directly loaded new Product Catalog SKU: ${prodSku}`, 'success');
              ...
              <div className="space-y-3 font-mono text-xs">
                ...
                <button 
                  onClick={() => {
                    ...
                    addLog('Marcus (AI Marketing)', 'Automated Offer Deployed', `Auto-injected discount codes matching campaign: "${campName}". Syncing metadata to shop headers.`, 'success');
          ...
          {activeCommand === 'customers' && (
            <div className="space-y-2">
              <span className="text-[10px] text-indigo-400 font-mono font-bold block uppercase tracking-wider">指令执行结果: 双优终身价值(LTV)客户排行 (Top 5)</span>
              
              <div className="bg-slate-950/60 border border-slate-850 rounded-xl overflow-hidden">
                <table className="w-full text-left font-mono text-[10px]">
                  <thead>
                    <tr className="bg-slate-900 text-slate-400 border-b border-slate-850">
                      <th className="p-2 font-bold">排名及金主企业</th>
                      <th className="p-2 font-bold">累计成交额</th>
                      <th className="p-2 font-bold text-center">忠诚积分</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-slate-900/60 hover:bg-slate-900/30">
                      <td className="p-2 text-slate-200">🥇 杭州高登时装有限公司</td>
                      <td className="p-2 font-bold font-mono text-emerald-400">¥ 120,400.00</td>
                      <td className="p-2 text-center text-indigo-400 font-bold">9,830 VP</td>
                    </tr>
                    <tr className="border-b border-slate-900/60 hover:bg-slate-900/30">
                      <td className="p-2 text-slate-200">🥈 上海优选极物馆</td>
                      <td className="p-2 font-bold font-mono text-emerald-400">¥ 89,320.00</td>
                      <td className="p-2 text-center text-indigo-400 font-bold">6,500 VP</td>
                    </tr>
                    <tr className="border-b border-slate-900/60 hover:bg-slate-900/30">
                      <td className="p-2 text-slate-200">🥉 北京朝阳外贸集采</td>
                      <td className="p-2 font-bold font-mono text-emerald-400">¥ 52,100.00</td>
                      <td className="p-2 text-center text-indigo-400 font-bold">4,120 VP</td>
                    </tr>
                    <tr className="border-b border-slate-900/60 hover:bg-slate-900/30">
                      <td className="p-2 text-slate-200">4. 极客数码海外买手部</td>
                      <td className="p-2 font-bold font-mono text-emerald-400">¥ 43,180.00</td>
                      <td className="p-2 text-center text-indigo-400 font-bold">3,500 VP</td>
                    </tr>
                    <tr className="border-b border-slate-900/60 hover:bg-slate-900/30">
                      <td className="p-2 text-slate-200">5. 深圳保税区供应链采购</td>
                      <td className="p-2 font-bold font-mono text-emerald-400">¥ 39,200.00</td>
                      <td className="p-2 text-center text-indigo-400 font-bold">3,100 VP</td>
                    </tr>
                  </tbody>
                </table>
              </div>
