...
import { IndustryType, TenantConfig, ProductItem, OrderItem, AIEmployee, Workflow, WorkflowNode, KnowledgeDoc, McpTool, AppMarketItem, CollaborationLog, SourcingRecommendation } from './types';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'command' | 'agents' | 'mcp' | 'knowledge' | 'marketplace' | 'sourcing' | 'visual-workflow' | 'doctree'>('command');
  ...
  const [collaborationLogs, setCollaborationLogs] = useState<CollaborationLog[]>([
    { id: '1', timestamp: '21:32:04', agent: 'Sophia (CEO)', action: 'Initialize Core systems', details: 'All SaaS multi-tenant isolation layers compiled securely.', type: 'success' },
    { id: '2', timestamp: '21:34:10', agent: 'Marcus (AI Marketing)', action: 'Optimize CPA limits', details: 'Analyzed organic ROAS on TikTok; synced parameters to Storefront.', type: 'tool' },
    { id: '3', timestamp: '21:35:12', agent: 'Oliver (Operations)', action: 'WMS Audit Triggered', details: 'Detected inventory shortages across primary distribution routes.', type: 'warning' }
  ]);
  ...
  // Dynamic system simulation logic: periodically add automated AI logs to show coordination!
  useEffect(() => {
    const intervals = [
      () => {
        const currentData = tenantDB[selectedIndustry];
        const lowStockCount = currentData.products.filter(p => p.stock <= p.minStockThreshold).length;
        if (lowStockCount > 0) {
          const names = ['Oliver', 'Stuart', 'Mia'];
          const randomName = names[Math.floor(Math.random() * names.length)];
          addLog(
            randomName,
            'Auto-Procurement Check',
            `Scanning alternative distributors for ${lowStockCount} low-stock material items. Safe-stock margin: 40%`,
            'tool'
          );
        }
      },
      () => {
        addLog(
          'Sophia (CEO)',
          'Metric Analytics Sync',
          `Completed hourly RAG aggregation. Calculated overall tenant performance efficiency. Operational cost optimal.`,
          'info'
        );
      },
      () => {
        const names = ['Marcus', 'Zoe', 'Dr. Jamie', 'Emily'];
        const randomName = names[Math.floor(Math.random() * names.length)];
        const comments = [
          'Meta CPM optimized by 4.2% after targeting local micro-segments.',
          'Cross-border digital invoice synced with global ledger rules.',
          'Customer FAQ search hit rate is 98.4%. Resolving pending claims inside 3.2s.',
          'Scheduled automated newsletter checking dormant client lists.'
        ];
        addLog(
          randomName,
    ...
    const timer = setInterval(() => {
      const selectedAction = intervals[Math.floor(Math.random() * intervals.length)];
      selectedAction();
    }, 15000); // every 15 seconds

    return () => clearInterval(timer);
  }, [selectedIndustry, tenantDB]);
  ...
  const handleTriggerSourcing = async () => {
    ...
    try {
      ...
      addLog('Sophia (AI CEO)', 'Market Sourcing Analysis Live', `Retrieved ${resData.recommendations?.length || 0} tailored SKU suggestions using Gemini real-time competitor intelligence.`, 'success');
    } catch (err: any) {
      console.error(err);
  ...
  const runVisualWorkflowSimulator = () => {
    ...
    const executeVisualStep = (index: number) => {
      ...
      const detailsMap: Record<string, string> = {
        ...
        'ai_decision': `[AI Employee Decision] Sophia consulted database. Oliver dispatched to optimize carrier routing and evaluate parcel shipping weight.`,
  ...
  const addVisualNode = (type: 'trigger' | 'ai_decision' | 'condition' | 'action') => {
    const typesMap = {
      ...
      ai_decision: { title: 'AI Assistant Reasoning Node', details: 'Sophia processes context using the Gemini API.' },
  ...
  const loadPresetWorkflow = (presetName: string) => {
    if (presetName === 'triage') {
      setVisualNodes([
        ...
        { id: 'p_2', type: 'ai_decision', status: 'idle', title: 'Evaluate Threat and Risk Status', details: 'Sophia maps fraud risk score heuristics.' },
      ...
      if (index >= wf.nodes.length) {
        ...
        setTimeout(() => {
          ...
          if (wf.id === 'wf_r1') { // Low stock auto procurement
            ...
            addLog('Oliver (AI Ops)', 'Automated PO Dispatched', 'Procured 60 units of SKU-R189 at 35% margin tier.', 'success');
            ...
            addLog('Emily (Customer care)', 'Fraud Shield Terminated', 'Verified third-party shipping stamps. Released partial compensation.', 'success');
            ...
            addLog('Marcus (Marketing)', 'Campaign Live', 'Injected 20% discount coupon fast-track tags on client app catalog.', 'success');
          } else {
  ...
  const triggerOrderAudit = (orderId: string) => {
    addLog('Emily (Customer Support)', 'Order Inspection Fired', `Performing active SLA audit with parcel courier for Order ${orderId}`, 'info');
    ...
    setTimeout(() => {
      ...
      addLog('Emily (AI Core)', 'Refund Claim Approved', `Courier digital coordinate matched customer zip. Verified risk clear. Issued order status update.`, 'success');
    }, 1500);
  };
  ...
  const executeStrategicDecision = (scenario: string) => {
    addLog('Sophia (CEO)', 'Execute Strategic Recommendation', `Applying dynamic operational adjustments for: ${scenario}`, 'tool');
    ...
    setTimeout(() => {
      if (scenario.includes('TikTok') || scenario.includes('Ads')) {
        ...
        addLog('Marcus (Marketing)', 'Ad Budget Scaled', 'Scaled Meta CPC bidding threshold matching our high-ROI time interval.', 'success');
        ...
        addLog('Emma', 'Bundle Offer Deployed', 'Dormant subscriber channels cleared. Sold 22 expiring Wagyu premium sets.', 'success');
      } else {
        ...
        if (lowStock) {
          ...
          addLog('Sophia (CEO)', 'Task Delegated', 'Core database healthy. Continued standard SLA metrics monitoring.', 'info');
        }
      }
    }, 1200);
  };
  ...
  if (viewMode === 'landing') {
    return (
      <LandingPage 
        ...
        onQuickBypass={() => {
          setViewMode('app');
          addLog('Sophia (CEO)', '一键快捷通道', '测试用户通过一键直达免签安全通道进入 SaaS 商家工作台后台！', 'success');
        }}
      />
    );
  }
  ...
  return (
    <div id="saas-platform-root" className="flex h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <aside id="saas-sidebar" className="w-64 bg-[#1a1b1d] text-[#e3e3e3] flex flex-col border-r border-[#2d2e30]">
        ...
        <div className="px-3.5 py-2.5 bg-[#121314]/50 border-b border-[#2d2e30] flex items-center justify-between">
          ...
          <select 
            ...
            className="bg-[#242426] border border-[#2d2e30] text-white text-[10px] rounded-lg p-1 font-semibold focus:outline-none cursor-pointer max-w-[80px]"
          >
            <option value="retail">👕 服装</option>
            <option value="food">🍔 外卖</option>
            <option value="education">🎓 网店</option>
            <option value="healthcare">🏪 门店</option>
        ...
        <nav id="sidebar-nav" className="flex-1 p-2 space-y-4 overflow-y-auto font-sans">
          
          {/* Section 1: Dashboard Suite */}
          <div className="space-y-0.5">
            <button 
              onClick={() => setActiveTab('command')}
              className={`w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-medium transition-all cursor-pointer ${activeTab === 'command' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <Home className="w-4 h-4 text-[#969696]" />
              <span>家</span>
            </button>
            ...
            <button 
              onClick={() => {
                ...
                addLog('Command Center', 'Interactive Filter', 'Filtered workplace overview for [All Open Orders]. Ready for processing.', 'info');
              }}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer hover:bg-[#242426] text-[#b5b5b5] hover:text-white`}
            >
              <div className="flex items-center gap-2.5">
                <ShoppingCart className="w-4 h-4 text-[#969696]" />
                <span>订单</span>
              </div>
              <span className="text-[9px] bg-rose-500/20 text-rose-450 font-bold px-1.5 py-0.2 rounded-full font-mono">{currentIndustryData.orders.length}</span>
            </button>

            <button 
              onClick={() => {
                setActiveTab('command');
                addLog('Command Center', 'Interactive Filter', 'Filtered workplace catalog matrix. Active SKU: ' + currentIndustryData.products.length, 'info');
              }}
              className={`w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-medium transition-all cursor-pointer hover:bg-[#242426] text-[#b5b5b5] hover:text-white`}
            >
              <ShoppingBag className="w-4 h-4 text-[#969696]" />
              <span>产品</span>
            </button>
            ...
            <button 
              onClick={() => {
                setActiveTab('command');
                addLog('Command Center', 'Analytics Filter', 'Rendered Customer list statistics. Active registered buyers: 32.', 'info');
              }}
              className={`w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-medium transition-all cursor-pointer hover:bg-[#242426] text-[#b5b5b5] hover:text-white`}
            >
              <Users className="w-4 h-4 text-[#969696]" />
              <span>顾客</span>
            </button>

            <button 
              onClick={() => setActiveTab('agents')}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'agents' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <div className="flex items-center gap-2.5">
                <Megaphone className="w-4 h-4 text-[#969696]" />
                <span>营销</span>
              </div>
              <span className="text-[9px] bg-indigo-500/20 text-indigo-400 font-bold px-1 py-0.2 rounded font-sans uppercase">Campaign</span>
            </button>

            <button 
              onClick={() => setActiveTab('visual-workflow')}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'visual-workflow' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <div className="flex items-center gap-2.5">
                <Percent className="w-4 h-4 text-[#969696]" />
                <span>折扣</span>
              </div>
              <span className="text-[8px] bg-emerald-500/20 text-emerald-400 font-bold px-1.5 py-0.2 rounded font-sans uppercase">Auto</span>
            </button>

            <button 
              onClick={() => setActiveTab('knowledge')}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'knowledge' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <div className="flex items-center gap-2.5">
                <FileText className="w-4 h-4 text-[#969696]" />
                <span>内容 (知识库)</span>
              </div>
              <span className="text-[10px] text-slate-500 font-mono font-bold">{currentIndustryData.knowledge.length}</span>
            </button>

            <button 
              onClick={() => setActiveTab('marketplace')}
              className={`w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-medium transition-all cursor-pointer ${activeTab === 'marketplace' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <Store className="w-4 h-4 text-[#969696]" />
              <span>市场 (应用市场)</span>
            </button>

            <button 
              onClick={() => setActiveTab('sourcing')}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'sourcing' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <div className="flex items-center gap-2.5">
                <Coins className="w-4 h-4 text-[#969696]" />
                <span>金融 (供应链)</span>
              </div>
              <span className="text-[8px] bg-[#d97706]/20 text-amber-500 font-bold px-1 py-0.2 rounded font-mono uppercase">AI</span>
            </button>

            <button 
              onClick={() => setActiveTab('doctree')}
              className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'doctree' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
            >
              <div className="flex items-center gap-2.5">
                <BarChart3 className="w-4 h-4 text-[#969696]" />
                <span>分析 (文档树)</span>
              </div>
              <span className="text-[10px] bg-indigo-500/10 text-indigo-400 font-bold px-1.5 py-0.5 rounded">91%</span>
            </button>
          </div>

          {/* Section 2: Sales Channels */}
          <div>
            <p className="text-[9px] font-bold text-[#7a7b7c] tracking-widest uppercase mb-1.5 px-2">销售渠道</p>
            <div className="space-y-0.5">
              <button 
                onClick={() => setIsOnlineStoreOpen(true)}
                className="w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium hover:bg-[#242426] text-[#b5b5b5] hover:text-white cursor-pointer group"
              >
                <div className="flex items-center gap-2.5">
                  <Globe className="w-4 h-4 text-[#969696]" />
                  <span>在线商店</span>
                </div>
                <ExternalLink className="w-3 h-3 text-[#7a7b7c] opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>

              <button 
                onClick={() => setActiveTab('agents')}
                className={`w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-medium transition-all cursor-pointer ${activeTab === 'agents' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
              >
                <Bot className="w-4 h-4 text-[#969696]" />
                <span>代理</span>
              </button>
            </div>
          </div>
          ...
          {/* Section 3: Apps */}
          <div>
            <p className="text-[9px] font-bold text-[#7a7b7c] tracking-widest uppercase mb-1.5 px-2">应用</p>
            <div className="space-y-0.5">
              <button 
                onClick={() => setActiveTab('mcp')}
                className={`w-full text-left p-2 rounded-lg flex items-center justify-between text-xs font-medium transition-all cursor-pointer ${activeTab === 'mcp' ? 'bg-[#2b2b2b] text-white font-bold' : 'hover:bg-[#242426] text-[#b5b5b5] hover:text-white'}`}
              >
                <div className="flex items-center gap-2.5">
                  <Cpu className="w-4 h-4 text-[#969696]" />
                  <span>MCP 控制中心</span>
                </div>
        ...
        {/* Bottom Menu: Sidekick and Document Sync & Settings */}
        <div id="sidebar-bottom" className="p-2 border-t border-[#2d2e30] bg-[#121314]/90 space-y-1 font-sans">
          
          <button 
            onClick={() => setIsCommandCenterOpen(true)}
            className="w-full text-left p-2 rounded-lg flex items-center gap-2.5 text-xs font-black text-indigo-400 hover:text-white hover:bg-gradient-to-r hover:from-indigo-950 hover:to-[#242426] transition-all cursor-pointer"
          >
            <Sparkles className="w-4 h-4 animate-pulse text-indigo-400" />
            <span>配角对话 (Sidekick Co)</span>
          </button>
      ...
      <main id="saas-main-viewport" className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div id="saas-split-pane" className="flex-1 flex overflow-hidden">
          ...
          <div id="saas-workspace" className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6">
            ...
            {/* TAB 2: AI EMPLOYEE FLEET DIRECT CONVERSATION */}
            {activeTab === 'agents' && (
              <div id="tab-employee-fleet" className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-fadeIn">
                
                {/* Agent Fleet Roster Cards */}
                <div className="xl:col-span-6 space-y-4">
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-1">
                    <h3 className="font-bold text-slate-800 font-display text-base">Select Specialist to Consult</h3>
                    <p className="text-xs text-slate-500">
                      These are the autonomous AI employees assigned to your SaaS tenant. Click on any model card to trigger direct real-time Gemini-grounded dialogue, or view their operational directives.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {currentIndustryAgents.map((agent) => {
                      const isFocused = chatAgent?.id === agent.id;
                      return (
                        <div 
                          key={agent.id}
                          onClick={() => openChatWithAgent(agent)}
                          className={`bg-white border text-left rounded-xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer relative flex flex-col justify-between ${isFocused ? 'ring-2 ring-indigo-500 border-indigo-500 scale-[1.01]' : 'border-slate-200'}`}
                        >
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-[10px] text-slate-400 font-semibold">{agent.title}</span>
                                </div>
                              </div>
                              <span className="flex items-center gap-1.5 bh-green-50 text-green-700 text-[9px] font-bold py-0.5 px-2 bg-slate-50 rounded-full">
                          ...
                          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                            <span className="text-[10px] text-slate-400">Model: <b className="font-mono text-slate-600">{agent.model}</b></span>
                            <span className="font-bold text-indigo-600 text-xs flex items-center gap-1 group-hover:underline">
                              Consult &rarr;
                            </span>
                          </div>
                        </div>
                ...
                {/* Live Chat Panel with Gemini */}
                <div className="xl:col-span-6 flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm h-[600px] overflow-hidden">
                  ...
                  {chatAgent ? (
                    <>
                      ...
                      <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <span className="text-2xl">{chatAgent.emoji}</span>
                          <div>
                            <span className="font-bold text-slate-900 text-sm font-display flex items-center gap-1.5">
                              {chatAgent.name}
                              <span className="text-[10px] bg-indigo-50 border border-indigo-100 text-indigo-700 rounded px-1.5 py-0.2">System Specialist</span>
                            </span>
                            <p className="text-[10px] text-slate-500 truncate max-w-[280px]">{chatAgent.title}</p>
                          </div>
                        </div>
                      ...
                      <div className="flex-1 overflow-y-auto p-5 space-y-4">
                        {(chatMessages[chatAgent.id] || []).map((msg, i) => {
                          const isAss = msg.role === 'assistant';
                          return (
                            <div 
                              key={i} 
                              className={`flex gap-3 max-w-[85%] ${isAss ? 'mr-auto text-left' : 'ml-auto flex-row-reverse text-right'}`}
                            >
                              <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center font-bold text-xs ${
                                isAss ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-white'
                              }`}>
                                {isAss ? chatAgent.emoji : <User className="w-4.5 h-4.5" />}
                              </div>
                              
                              <div className={`p-3.5 rounded-2xl text-xs leading-relaxed space-y-1 ${
                                isAss ? 'bg-slate-100 text-slate-800 border border-slate-200' : 'bg-indigo-600 text-white'
                              }`}>
                                {/* Small strategic layout container matching chat spacing */}
                        ...
                        {chatLoading && (
                          <div className="flex gap-3 max-w-[80%] mr-auto text-left items-center">
                            <div className="w-7 h-7 rounded-sm bg-indigo-100 text-indigo-700 shrink-0 flex items-center justify-center font-bold text-xs animate-bounce">
                              {chatAgent.emoji}
                            </div>
                            <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg text-xs text-slate-500 flex items-center gap-2 font-mono">
                              <span className="w-2 h-2 rounded-full bg-indigo-600 animate-ping"></span>
                              <span>{chatAgent.name} is reading active store parameters and reasoning via Gemini...</span>
                            </div>
                          </div>
                        )}
                      ...
                      {/* Post form chat */}
                      <form onSubmit={handleSendChatMessage} className="p-4 border-t border-slate-100 flex items-center gap-2 shrink-0">
                        <input 
                          type="text"
                          required
                          value={chatInput}
                          onChange={(e) => setChatInput(e.target.value)}
                          placeholder={`Instruct ${chatAgent.name}... (e.g., "Auditing stock levels" or "Optimize TikTok price")`}
                          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
                        />
                        <button 
                          type="submit"
                          disabled={chatLoading}
                          className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white p-2.5 rounded-xl transition-all font-bold shrink-0 shadow-sm"
                        >
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-400 space-y-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-600 font-display">No Representative Selected</p>
                        <p className="text-xs text-slate-400 mt-1 max-w-[280px]">
                          Select an agent in the active fleet to coordinate or execute operations using live state telemetry.
                        </p>
                      </div>
                    </div>
                  )}
            ...
            {activeTab === 'sourcing' && (
              <div id="tab-sourcing-recommender" className="space-y-6 animate-fadeIn text-left">
                {/* Header card info */}
                <div className="bg-gradient-to-r from-indigo-900 to-slate-900 text-white rounded-2xl p-6 shadow-md border border-slate-800 relative overflow-hidden">
            ...
            {activeTab === 'visual-workflow' && (
              <div id="tab-visual-workflows-panel" className="space-y-6 animate-fadeIn text-left">
                ...
                <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-2xl p-6 shadow-md border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="space-y-1">
                          ...
                          return (
                            <div className="space-y-4 text-xs font-normal">
                              ...
                              <div>
                                ...
                                <select
                                  value={editingNode.type === 'trigger' ? 'order_placed' : (editingNode.type === 'ai_decision' ? 'sophia' : 'dhl')}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    const updatedDetails = val === 'order_placed' ? 'Fires automatically upon checkout confirmation.' : (val === 'sophia' ? 'Sophia reads database state via Gemini Flash SDK.' : 'Dispatches logistic REST webhook automatically.');
                                ...
                                >
                                  ...
                                  {editingNode.type === 'ai_decision' && (
                                    <>
                                      <option value="sophia">Sophia AI CEO General Router</option>
                                      <option value="oliver">Oliver AI WMS Procurement Specialist</option>
                                      <option value="stuart">Stuart AI CRM Support Agent</option>
                                    </>
                                  )}
          ...
          {/* Right Pane: Real-time Enterprise multi-agent logs feed & Slack simulation */}
          <div id="saas-right-log-drawer" className="hidden xl:flex w-80 bg-slate-900 border-l border-slate-800 flex-col overflow-hidden shrink-0">
            
            {/* Log Panel Header */}
            <div className="p-4 border-b border-slate-800 bg-slate-950 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Terminal className="w-4 h-4 text-indigo-400" />
                <span className="font-bold text-xs text-white uppercase tracking-wider">Enterprise Operation Logs</span>
              </div>
            ...
            <div className="p-3 bg-slate-950 text-left">
              <p className="text-[10px] text-slate-500 leading-normal">
                Shows collaboration logs between the AI CEO and special agents. Runs workflows automatically to keep operations active.
              </p>
            </div>

            {/* Simulated Live logs stream */}
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 font-mono text-[11px] leading-relaxed">
              {collaborationLogs.map((log) => {
                let badgeColor = "text-indigo-450 bg-indigo-950/40 border border-indigo-900/60";
                if (log.type === 'success') badgeColor = "text-emerald-400 bg-emerald-950/40 border border-emerald-900/40";
                if (log.type === 'warning') badgeColor = "text-amber-405 bg-amber-950/40 border border-amber-900/40";
                if (log.type === 'tool') badgeColor = "text-violet-400 bg-violet-950/40 border border-violet-900/40";
                if (log.type === 'error') badgeColor = "text-rose-400 bg-rose-950/40 border border-rose-900/40";

                return (
                  <div key={log.id} className="p-3 bg-slate-950 border border-slate-850/60 rounded-lg space-y-1.5 hover:bg-slate-950/80 transition-all">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 text-[10px]">{log.timestamp}</span>
                      <span className="font-bold text-indigo-300 text-[10px] truncate max-w-[140px]">{log.agent}</span>
                    </div>

                    <div className="text-white font-semibold flex items-center gap-1.5 leading-snug">
                      <span className={`px-1.5 py-0.2 rounded font-semibold text-[8px] tracking-wider uppercase ${badgeColor}`}>
                        {log.action}
                      </span>
                    </div>

                    <p className="text-slate-400 leading-normal font-normal text-[10px]">
                      {log.details}
                    </p>
                  </div>
                );
            ...
            {/* Platform indicators footer */}
            <div className="p-3.5 border-t border-slate-800 bg-slate-950 text-[10px] text-slate-500 space-y-2">
              <div className="flex justify-between">
                <span>Total active SaaS Stores:</span>
                <span className="text-slate-300 font-bold">{PLATFORM_STATS.activeTenants}</span>
              </div>
              <div className="flex justify-between">
                <span>Autonomous Saved Fees:</span>
                <span className="text-emerald-400 font-bold">${PLATFORM_STATS.totalSavedFees.toLocaleString()} USD</span>
              </div>
            </div>
      ...
      {/* AI command center executive panel drawer */}
      <AICommandCenter
